...
def test_trading_script_dry_run_without_credentials(monkeypatch, capsys):
    """The trading harness should fall back to synthetic data when credentials are absent."""

    monkeypatch.delenv("ALPACA_API_KEY", raising=False)
    monkeypatch.delenv("ALPACA_SECRET_KEY", raising=False)
    monkeypatch.setenv("HURRICANE_LOG_LEVEL", "ERROR")

    from hurricane_spy.scripts import run_trading
    ...
    assert "Data source: synthetic bundle" in output
    assert "Order submission skipped" in output
