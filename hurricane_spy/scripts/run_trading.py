...
def main() -> None:
    ...
    try:
        ...
        if not args.dry_run:
            raise
        LOGGER.info(
            "Alpaca credentials not found (%s); running in synthetic dry-run mode",
            exc,
        )
        credentials = None

    if credentials is None:
        periods = max(int(lookback.total_seconds() // 60) or 1, 500)
        index = pd.date_range(end=datetime.now(UTC), periods=periods, freq="min")
        bundle = generate_dummy_data(index, seed=args.seed)
        live = False
        credentials = AlpacaCredentials(api_key="DUMMY", secret_key="DUMMY")
    else:
